...
func (s *defaultCatalogService) UpdateVariantInventory(ctx context.Context, sku string, qty int, status domain.StockStatus) error {
	if qty < 0 {
		return fmt.Errorf("%w: stock quantity must not be negative", ErrInvalidVariant)
	}
	if !status.IsValid() {
		return fmt.Errorf("%w: invalid stock status", ErrInvalidVariant)
	}
	return s.variants.UpdateVariantInventory(ctx, sku, qty, status)
}
